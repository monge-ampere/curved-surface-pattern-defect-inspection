---
layout: post
title: "第二讲：曲面 Pattern 缺陷检测的本质：结构化归一化与缺陷测量"
...
*图 1：曲面 Pattern 检测中两类最基本、也最容易被低估的难点。左图展示了曲面成像带来的明显形变；右图展示了不同国别 Pattern 之间的巨大差异。更关键的是，这套系统并不是针对单一版式临时写出的专用程序，而是在耳机与充电盒、不同国别、乃至不同产品之间尽量保持同一套核心 DLL 不变，只通过图档、XML、INI 等外部描述层完成适配。这也正是它能够走向产品化和跨场景推广的前提。*

从这两张图其实就能看出，这个问题一开始就不是“把某几个字符认出来”那么简单。左图说明，同样的结构对象只要落到曲面成像条件下，局部形态就会出现明显变化；右图则说明，系统面对的并不是单一版式，而是大量国别 Pattern、图样组合乃至产品对象本身的变化。也就是说，这套系统既要处理曲面带来的成像与几何变化，又要在同一核心算法内核下尽量覆盖不同国别、不同图档、甚至不同产品对象。这正是为什么后面必须引入模板、配准、参数链和配置链，而不能把它理解成普通字符检测。
...
![Typical defect examples from curved-surface pattern inspection]({{ "/assets/images/inspection_defect_detection_examples.png" | relative_url }})
