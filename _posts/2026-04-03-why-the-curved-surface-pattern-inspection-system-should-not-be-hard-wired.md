...
也就是说，插件不是一个游离在系统之外的外挂，而是会通过 `XML` 与 `block` 配置进入主系统的运行框架。某个对象一旦被配置为插件检测路径，那么它就不再走标准的主检测链，而是被路由到对应的插件模块中去完成局部分析。
...
1. `XML` 中对插件信息本身的配置；
2. `block` 标签中通过 `methods="1"` 等方式，把当前 `block` 路由到插件检测路径。

所以，插件机制之所以成立，不只是因为 `DLL` 能动态加载，而是因为它已经进入了 `XML` 与 `block` 的配置组织层。

如果进一步借用设计模式的语言来描述，这一层其实并不只是“动态加载一个 DLL”，而更接近于几种经典思想的组合。

首先，它带有明显的**工厂机制**：主系统并不在编译期写死二维码、条码、规则 `logo` 或 `AI` 模块的具体实现，而是根据配置在运行时装配对应插件，并暴露统一分析入口。  
其次，它又具有典型的**策略模式**特征：虽然不同插件内部算法完全不同，但在系统边界上，它们都被看作“当前 `ROI` 上的一种局部检测策略”。主系统真正依赖的，不是插件内部怎么做，而是统一的输入输出协议。  
再进一步说，这种做法本质上也是一种**运行时绑定**和**依赖倒置**：高层检测框架不直接依赖对象特定实现，具体算法反而通过统一插件边界回接主系统。

这里需要顺手说明一下：如果严格按软件设计模式的经典定义去看，这一层并不完全等同于“抽象工厂（Abstract Factory）”，因为它并没有去生成一整族相关对象；但从工程思想上看，它确实具有一种非常明确的工厂式边界：**把对象特定算法的装配，与主系统对这些算法的使用清楚地分开。**
...
```
这段代码表面上只是 `DLL` 加载，但从系统边界看，它真正做的是两件事。

第一，它把“插件如何被找到、被装配、被注册”统一了起来。主系统不需要在主链代码里到处写特例判断，而是通过统一装配入口，把对象特定算法接入当前运行框架。

第二，它把“插件内部算法是什么”和“主系统如何使用插件”彻底分开了。对主系统来说，二维码插件、条码插件、规则 `logo` 插件，甚至后面的 `AI` 插件，最终都只是统一边界上的一个 `analysis` 入口。被统一起来的不是算法内部细节，而是系统边界上的输入输出协议。

所以，这一层最值得强调的，不是 Windows API，也不是“动态加载”这几个字本身，而是：

> 插件机制通过一种带有工厂思想的运行时策略装配方式，把主系统框架和对象特定算法清楚地分开了。
